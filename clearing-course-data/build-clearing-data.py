...
KEEP IN LOCK-STEP WITH process.html
-----------------------------------
process.html is a browser-only port of the validation rules, record shaping and
CMS HTML output below (the upload-and-copy tool on webstage). If you change any
validation rule, the record shaping, or the CMS <article> markup here, mirror the
change in process.html and re-run the parity test: `python3 tools/parity_test.py`.
"""
